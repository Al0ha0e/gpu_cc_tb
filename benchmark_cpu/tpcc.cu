...
AAAABBB *Pre(AAA000 *aaa000)
{
    ...
    str_opts.push_back("-arch=compute_80");
...
AAA000 *PaymentPre0()
{
    ...
    std::string file_name = std::string("tpcc_pm_wh") + std::to_string(warehouse_cnt) + ".txs";
...
AAA000 *NewOrderPre0()
{
    ...
    std::string file_name = std::string("tpcc_no_wh") + std::to_string(warehouse_cnt) + ".txs";
    FILE *file = fopen(file_name.c_str(), "rb");
