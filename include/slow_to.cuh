...
namespace cc
{
    ...
    struct SlowWriteInfo
    {
        to_full_timestamp_t *tsp;
        to_full_timestamp_t prev_ts;
        void *srcdata;
        void *dstdata;
        size_t size;
    };

    struct DynamicSlowTOInfo
    {
        SlowWriteInfo *write_info;
        char *has_wts;

        __host__ __device__ DynamicSlowTOInfo() {}
        __host__ __device__ DynamicSlowTOInfo(SlowWriteInfo *write_info,
...
#define TO_LATCH_TABLE 0
    ...
    public:
        ...
        __device__ Slow_TO_GPU(void *txs_info, void *info, size_t tid)
        {
            ...
            DynamicSlowTOInfo *tinfo = (DynamicSlowTOInfo *)info;
...
#pragma unroll
            for (int i = 0; i < WCNT; i++)
            {
                ...
                to_full_timestamp_t *entry = winfo.tsp;
...
#ifdef TX_DEBUG
            common::AddEvent(self_events + RCNT + WCNT, 0, self_tid, 2);
#endif
            return true;
        }
        ...
        {
            unsigned long long manager_st_time = clock64();
            volatile to_full_timestamp_t *entry = ((volatile to_full_timestamp_t *)SLOW_TO_TS_TABLE) + obj_idx;
            int *latch_entry = ((int *)TO_LATCH_TABLE) + obj_idx;
            while (true)
            {
                latch_lock(latch_entry);
                unsigned long long wait_st_time = clock64();
                if (entry->wts > self_ts)
                {
                ...
                // ts2.s.rts = self_ts;
                entry->rts = max(entry->rts, (unsigned long long)self_ts);
                memcpy(dstdata, srcdata, size);
#ifdef TX_DEBUG
                common::AddEvent(self_events + tx_idx, obj_idx, self_tid, 0);
#endif
                latch_unlock(latch_entry);
                break;
            }
            ...
            size_t size)
        {
            ...
            latch_lock(latch_entry);
            if (entry->rts > self_ts || entry->wts > self_ts || entry->uncommited)
            {
                latch_unlock(latch_entry);
                rollback();
                return false;
            ...
            winfo->prev_ts = *(to_full_timestamp_t *)entry;
            entry->uncommited = 1;
            entry->wts = self_ts;
            entry->rts = self_ts;
#ifdef TX_DEBUG
            common::AddEvent(self_events + RCNT + tx_idx, obj_idx, self_tid, 1);
#endif
            latch_unlock(latch_entry);

            winfo->tsp = (to_full_timestamp_t *)entry;
...
#pragma unroll
            for (int i = 0; i < WCNT; i++)
            {
                ...
                {
                    SlowWriteInfo &winfo = write_info[i];
                    int *latch_entry = (int *)TO_LATCH_TABLE +
                                       (winfo.tsp - (to_full_timestamp_t *)SLOW_TO_TS_TABLE);
                    latch_lock(latch_entry);
                    *winfo.tsp = winfo.prev_ts;
                    latch_unlock(latch_entry);
                }
            }
            __threadfence();
    ...
    public:
        ...
        {
            cudaMalloc(&ts_table, sizeof(to_full_timestamp_t) * db->table_st[db->table_cnt]);
            ...
            if (dynamic)
            {
                ...
                DynamicSlowTOInfo *tmp = new DynamicSlowTOInfo(write_info, has_wts);
                cudaMalloc(&to_gpu_info, sizeof(DynamicSlowTOInfo));
                cudaMemcpy(to_gpu_info, tmp, sizeof(DynamicSlowTOInfo), cudaMemcpyHostToDevice);
                delete tmp;
            }
            else
            {
        ...
        void Init(int batch_id, int batch_st) override
        {
            ...
            cudaMemset(ts_table, 0, sizeof(to_full_timestamp_t *) * db_cpu->table_st[db_cpu->table_cnt]);
        ...
        void GetCompileOptions(std::vector<std::string> &opts) override
        {
            ...
            opts.push_back("-D TO_LATCH_TABLE=" + std::to_string((unsigned long long)latch_table));
        ...
        {
            size_t common_sz = sizeof(to_full_timestamp_t) * db_cpu->table_st[db_cpu->table_cnt];
            if (dynamic)
            {
                common::DynamicTransactionSet_CPU *dtx = (common::DynamicTransactionSet_CPU *)info;
                return common_sz + sizeof(int) * tx_cnt * 2 + sizeof(size_t) * (tx_cnt + 1) + sizeof(char) * dtx->GetTotW() + sizeof(SlowWriteInfo) * dtx->GetTotW() + sizeof(DynamicSlowTOInfo);
            }
            return common_sz;
        }
    };
